...
public class VirgilioControllerScript : MonoBehaviour
{
	...
	public float defaultDirection = 0.0f;
	...
	static public bool alwaysrun = false;      //variabile globale per il livello 1
	...
	void FixedUpdate ()
	{
		Ray ray1 = new Ray(transform.position + Vector3.up, -Vector3.up);
		RaycastHit hitInfo1 = new RaycastHit();
		
		if (Physics.Raycast(ray1, out hitInfo1))
		{
			if(Vector3.Dot(Vector3.up,hitInfo1.normal)<=0.4f){
				anim.speed = 0.0f;
		...
		if (Mathf.Abs ((mainCharacter.gameObject.transform.position.x - gameObject.transform.position.x)) >= 20f || 
		    Mathf.Abs ((mainCharacter.gameObject.transform.position.y - gameObject.transform.position.y)) >= 20f) {
			Vector3 mainCharacterPosition = mainCharacter.transform.position;
			mainCharacterPosition.x -= 0.3f;
			mainCharacterPosition.y -= 0.3f;
			gameObject.transform.position = mainCharacterPosition;
		}
